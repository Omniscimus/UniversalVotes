		...
		if(args.length == 0) {
			...
			if(sender instanceof ConsoleCommandSender || sender.hasPermission("universalvotes.admin")) {
				if(args[0].equalsIgnoreCase("help") || args[0].equalsIgnoreCase("h") || args[0].equalsIgnoreCase("?")) {
					...
					if(plugin.getVotifierEnabled()) VoteListener.fakeVote(plugin, sender);
					else sender.sendMessage(ChatColor.RED + "Can't send a fake vote; Votifier isn't enabled in the UniversalVotes config!");
				}
			}
